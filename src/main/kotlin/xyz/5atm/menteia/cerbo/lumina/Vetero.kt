...
object Vetero {
    ...
    suspend fun lurina(lokoArbo: SintaksoArbo, datoArbo: SintaksoArbo): String {
        ...
        val lumina = VeteraDatumo.kodoj[raporto.weather[0].id]
                ?: throw Exception("${raporto.weather[0].id} ne ekzistas")
        println(raporto)
        return "luvana ${lumina} nevum ${Nombroj.nombrigi(raporto.main.temp.roundToInt())} posetim ${Nombroj.nombrigi((raporto.wind.speed * 3.6).roundToInt())}"
    }
}